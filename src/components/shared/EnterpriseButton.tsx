...
export default function EnterpriseButton({
  ...
  const variants = {
    primary:
      'bg-slate-950 text-white border-slate-950 shadow-[0_16px_38px_rgba(15,23,42,0.18)] hover:bg-indigo-700 hover:border-indigo-700',
    secondary:
      'bg-white/82 text-slate-900 border-slate-200 shadow-sm hover:border-cyan-200 hover:text-cyan-700 hover:bg-white',
    ghost:
  ...
  return (
    <Link
      href={href}
      className={cn(
        'group/enterprise-button inline-flex items-center justify-center gap-2 rounded-full border px-6 py-3 text-sm font-black transition-all duration-300',
        variants[variant],
        className
      )}
    >

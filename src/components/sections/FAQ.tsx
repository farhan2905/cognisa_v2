        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }} transition={{ delay: 0.2, duration: 0.7 }} className="flex flex-col gap-4">
          {faqs.map((faq, index) => {
            ...
            return (
              <div
                key={index}
                className={`relative overflow-hidden rounded-2xl transition-all duration-300 backdrop-blur-2xl border ring-1 shadow-[0_8px_32px_rgba(31,38,135,0.08),inset_0_1px_0_rgba(255,255,255,0.35)] ${
                  isOpen
                    ? 'bg-white/20 border-indigo-400/60 ring-indigo-500/30'
                    : 'bg-white/15 border-indigo-300/50 ring-indigo-400/20 hover:bg-white/20 hover:border-indigo-400/60 hover:ring-indigo-500/30'
                }`}
              >
